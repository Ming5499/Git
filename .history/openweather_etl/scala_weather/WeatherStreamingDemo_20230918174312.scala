...
object WeatherStreamingDemo {
  def main(args: Array[String]): Unit = {
    ...
    val weather_detail_df_4 = weather_detail_df_3.withColumn("CreationDate", to_date(weather_detail_df_3("CreationTime")).cast(DateType))
    ...
    val weather_detail_write_stream = weather_detail_df_5
      ...
      .outputMode("append")
      // Remove the outputMode() option, as it is not supported by the console format
      .format("console")
      .start()

    // Write final result to HDFS
